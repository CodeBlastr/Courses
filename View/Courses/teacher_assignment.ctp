...
<?php echo $this->Form->create('Task', array('type' => 'file'));?>
	<fieldset>
		...
		<div class="row">
			<div class="col-sm-12">
				<?php //echo $this->Form->input('Task.parent_id', array('empty' => true, 'label' => 'Which task list should this be on?')); ?>
				<?php echo $this->Form->input('Task.foreign_key', array('options' => $parentCourses, 'value' => $course_id, 'empty' => '- Select Course -', 'label' => 'Course Assignment Belongs to', 'class' => 'required')); ?>
			</div>
		</div>
		<?php echo $this->Form->input('Task.settings', array('type' => 'select', 'label' => 'Assignment Type', 'options' => $assignmentTypes, 'empty' => '-- Choose a Category --')); ?>
		<?php echo $this->Form->input('Task.description', array('type' => 'richtext')); ?>
		<?php echo $this->Form->input('Task.assignee_id', array('label' => 'Privacy', 'options' => array(0 => 'All Course Members', $this->Session->read('Auth.User.id') => 'Just Me'))); ?>
		<?php echo $this->Form->hidden('Task.model', array('value' => 'Course')); ?>
	</fieldset>
